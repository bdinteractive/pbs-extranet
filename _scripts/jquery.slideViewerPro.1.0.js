...
jQuery.fn.slideViewerPro = function(settings) {
	...
	return this.each(function(){
		...
		/* fix the width and height of the thumbnails */
		var thumbsWidth = 66;
		var thumbsHeight = 59; 
    ...
    .wrap(jQuery("<div style='width:"+ pictWidth +"px; overflow: hidden; position: relative; top: 0; left: 0'>"));
		...
		container.each(function(i) {
			...
			jQuery(this).find("img").each(function(z) {
				...
				var spanTitle = "<strong class='strongtitle'>"+jQuery(this).attr("title")+"</strong><br>"+jQuery(this).attr("alt");
				
				jQuery(this).after("<span class='typo' style='position: absolute; width:"+(pictWidth-12)+"px; margin: 0 0 0 -"+pictWidth+"px'>"+spanTitle+"<\/span>");
			});
			}
			...
			jQuerybtr.bind("click", function(){
					...
					// return to the first thumbnail
					if( jQuerybtr.hasClass('r_dis') ){
						jQuery(this).prev().prev().find("ul:not(:animated)").animate({ left: -(thumbsWidth+6+settings.thumbsRightMargin)*0}, 500, settings.easeFunc, function(){;});
					...
					r_enabled = false;
					jQuery(this).prev().prev().find("ul:not(:animated)").animate({ left: -(thumbsWidth+6+settings.thumbsRightMargin)*pos}, 500, settings.easeFunc, function(){authorityMixing();});		
			...
			jQuerybtl.bind("click", function(){	
					...
					l_enabled = false;
					jQuery(this).prev().find("ul:not(:animated)").animate({ left: -(thumbsWidth+6+settings.thumbsRightMargin)*pos}, 500, settings.easeFunc, function(){authorityMixing();});			
					return false;
			});						
			...
			jQuery("div#thumbSlider" + j).css({position: "relative", left: contBorder, top: settings.thumbsTopMargin+"px", width: settings.thumbs*(thumbsWidth+6)+((settings.thumbsRightMargin*settings.thumbs)-settings.thumbsRightMargin), height: thumbsHeight + 6, textAlign: "center", overflow: "hidden", margin: "0 auto"});
			jQuery("div#thumbSlider" + j + " ul").css({width: ((thumbsWidth+6)*pictEls)+settings.thumbsRightMargin*pictEls, position: "relative", left: "0", top: "0"});
